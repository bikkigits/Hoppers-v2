...
export const NearbyFilterBar: React.FC<Props> = ({
  ...
  return (
    <div
      ...
      className="w-full max-w-lg mx-auto overflow-hidden pointer-events-auto px-2 py-1"
    >
      <div
        ref={scrollContainerRef}
        className="flex items-center gap-1.5 overflow-x-auto hide-scrollbar no-scrollbar px-2 py-1 scroll-smooth"
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-all duration-150 active:scale-95 shadow-xs backdrop-blur-md shrink-0 border ${
                isActive
                  ? 'bg-amber-400 text-slate-950 border-amber-400 font-bold shadow-amber-500/20'
                  : 'bg-slate-900/85 text-slate-300 border-slate-800/90 hover:border-slate-700 hover:text-white'
              }`}
            >
